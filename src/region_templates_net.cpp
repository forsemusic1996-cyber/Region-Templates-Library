...
void CurlLock(CURL *, curl_lock_data, curl_lock_access, void *) { g_curl_mutex.lock(); }
void CurlUnlock(CURL *, curl_lock_data, curl_lock_access, void *) { g_curl_mutex.unlock(); }

bool IsCancelled(const std::shared_ptr<Job> &job) { return g_stopping.load() || job->cancel.load(); }
...
{
  if (IsCancelled(job)) { job->state = kFailure; SetError(job, "cancelled"); return; }
  const fs::path target(job->target);
  const fs::path parent = target.parent_path();
  std::error_code ec;
  if (!parent.empty()) fs::create_directories(parent, ec);
  if (ec) { job->state = kFailure; SetError(job, "cannot create cache directory: " + ec.message()); return; }
  const fs::path temporary = target.string() + ".part." + std::to_string(job->id);
  std::ofstream output(temporary, std::ios::binary | std::ios::trunc);
  if (!output) { job->state = kFailure; SetError(job, "cannot open temporary download file"); return; }
  CURL *curl = curl_easy_init();
  if (!curl) { output.close(); fs::remove(temporary, ec); job->state = kFailure; SetError(job, "curl initialization failed"); return; }
  ...
  if (result != CURLE_OK) {
    ...
    if (response_code) message += " (HTTP " + std::to_string(response_code) + ")";
    job->state = kFailure; SetError(job, std::move(message)); return;
  }
  if (IsCancelled(job)) { fs::remove(temporary, ec); job->state = kFailure; SetError(job, "cancelled"); return; }
  fs::remove(target, ec); ec.clear(); fs::rename(temporary, target, ec);
  if (ec) { fs::remove(temporary, ec); job->state = kFailure; SetError(job, "cannot commit downloaded file: " + ec.message()); return; }
  job->state = kSuccess;
}
...
void WorkerLoop()
{
  while (true) {
    std::shared_ptr<Job> job;
    {
      ...
      g_queue_wake.wait(lock, [] { return g_stopping.load() || !g_queue.empty(); });
      if (g_stopping.load() && g_queue.empty()) return;
      job = std::move(g_queue.front()); g_queue.pop();
    }
    RunJob(job);
  }
}
...
{
  g_stopping = false;
  for (size_t i = 0; i < kWorkerCount; ++i) g_workers.emplace_back(WorkerLoop);
}

void StopWorkers()
{
  g_stopping = true;
  { std::lock_guard<std::mutex> lock(g_jobs_mutex); for (auto &[id, job] : g_jobs) job->cancel = true; }
  g_queue_wake.notify_all();
  for (auto &worker : g_workers) if (worker.joinable()) worker.join();
  g_workers.clear();
  std::queue<std::shared_ptr<Job>> empty; std::swap(g_queue, empty);
}
...
{
  if (!url || !target || !*url || !*target || g_stopping.load()) return -1;
  auto job = std::make_shared<Job>();
  job->id = g_next_id.fetch_add(1); job->url = url; job->target = target;
  { std::lock_guard<std::mutex> lock(g_jobs_mutex); g_jobs.emplace(job->id, job); }
  { std::lock_guard<std::mutex> lock(g_queue_mutex); g_queue.push(job); }
  g_queue_wake.notify_one(); return static_cast<double>(job->id);
}
...
double HttpWait(double id)
{
  const auto job = FindJob(static_cast<int>(id));
  if (!job) return -1;
  while (job->state.load() == kPending) { if (g_stopping.load()) return -1; std::this_thread::sleep_for(std::chrono::milliseconds(5)); }
...
char *HttpError(double id)
{
  ...
  const auto job = FindJob(static_cast<int>(id));
  if (!job) { result = "unknown job"; return result.data(); }
  std::lock_guard<std::mutex> lock(job->mutex); result = job->error; return result.data();
}

double HttpCancel(double id)
{
  const auto job = FindJob(static_cast<int>(id));
  if (!job) return 0; job->cancel = true; return 1;
}
...
void *VarargHttpStart(void **args, int count) { g_vararg_result = (args && count >= 2 && args[0] && args[1]) ? HttpStart(static_cast<char *>(args[0]), static_cast<char *>(args[1])) : -1.0; return &g_vararg_result; }
void *VarargHttpPoll(void **args, int count) { g_vararg_result = (args && count >= 1 && args[0]) ? HttpPoll(*static_cast<double *>(args[0])) : -1.0; return &g_vararg_result; }
void *VarargHttpError(void **args, int count) { return (args && count >= 1 && args[0]) ? HttpError(*static_cast<double *>(args[0])) : const_cast<char *>("invalid Region Templates HTTP job"); }
void *VarargHttpWait(void **args, int count) { g_vararg_result = (args && count >= 1 && args[0]) ? HttpWait(*static_cast<double *>(args[0])) : -1.0; return &g_vararg_result; }
void *VarargHttpCancel(void **args, int count) { g_vararg_result = (args && count >= 1 && args[0]) ? HttpCancel(*static_cast<double *>(args[0])) : 0.0; return &g_vararg_result; }
void *VarargHttpRelease(void **args, int count) { g_vararg_result = (args && count >= 1 && args[0]) ? HttpRelease(*static_cast<double *>(args[0])) : 0.0; return &g_vararg_result; }
...
extern "C" REAPER_PLUGIN_DLL_EXPORT int REAPER_PLUGIN_ENTRYPOINT(REAPER_PLUGIN_HINSTANCE, reaper_plugin_info_t *rec)
{
  if (rec) {
    ...
    if (!g_share) { curl_global_cleanup(); return 0; }
    ...
    StartWorkers(); RegisterApi(true); return 1;
  }
  if (g_register) RegisterApi(false);
  StopWorkers();
  if (g_share) { curl_share_cleanup(g_share); g_share = nullptr; }
  curl_global_cleanup(); g_register = nullptr; return 0;
}
